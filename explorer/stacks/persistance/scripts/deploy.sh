...
COMPOSE_FILE=${COMPOSE_FILE:-../persistance.yml}
...
printf "%s" "${DATABASE_PASSWORD}" \
    | docker secret create "explorer_database_password_${STAGE}" - > /dev/null || exit
printf "%s" "${DATABASE_USERNAME}" \
    | docker secret create "explorer_database_username_${STAGE}" - > /dev/null || exit
printf "%s" "${DATABASE_SCHEMA}" \
    | docker secret create "explorer_database_schema_${STAGE}" - > /dev/null || exit
